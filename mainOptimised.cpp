...
float RayTrace(Ray ray, Shape* scene, Light* lights, int depth = 64)
{
	//escape
	if (depth == 0)
		return 0.0f;
...
void renderPoint(int x, int y, Camera* camera,Shape* scene, Light* lights, Screen* scr)
{
...
int main()
{
	...
	while (true)
	{
		...
		if (GetKeyState(0x57) & 0x8000)		position += scale * (1.0f / 30.0f) * camera.getForward();		//W
		if (GetKeyState(0x41) & 0x8000)		position -= scale * (1.0f / 30.0f) * camera.getRight();			//A
		if (GetKeyState(0x53) & 0x8000)		position -= scale * (1.0f / 30.0f) * camera.getForward();		//S
		if (GetKeyState(0x44) & 0x8000)		position += scale * (1.0f / 30.0f) * camera.getRight();			//D

		if (GetKeyState(VK_SHIFT) & 0x8000)	position.z -= scale * (1.0f / 30.0f);
		if (GetKeyState(VK_SPACE) & 0x8000)	position.z += scale * (1.0f / 30.0f);

		//CAMERA MOVEMENT
		if (GetKeyState(VK_UP) & 0x8000)	beta += scale * 1.0f / 120.0f / 5.0f * (2.0f * PI);
		if (GetKeyState(VK_LEFT) & 0x8000)	alfa -= scale * 1.0f / 120.0f / 5.0f * (2.0f * PI);
		if (GetKeyState(VK_DOWN) & 0x8000)	beta -= scale * 1.0f / 120.0f / 5.0f * (2.0f * PI);
		if (GetKeyState(VK_RIGHT) & 0x8000)	alfa += scale * 1.0f / 120.0f / 5.0f * (2.0f * PI);
...
#pragma omp parallel for
		for (int i = 0; i < HEIGHT * WIDTH; i++)
			renderPoint(i/HEIGHT, i%HEIGHT, &camera, &scene, &lights, &scr);
		//DRAWING
		scr.Draw();
	}
	return 0;
